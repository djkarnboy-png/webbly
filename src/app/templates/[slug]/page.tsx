...
export default async function TemplateDetailsPage({ params }: PageProps) {
    ...
    if (error) {
      return (
        <section className="app-page px-5 py-24 text-center sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold text-slate-50">Template unavailable</h1>
  ...
  return (
    <>
          ...
          <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-start">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-md border border-blue-400/20 bg-blue-500/10 px-2.5 py-1 text-xs font-semibold text-blue-300">
                  {template.category}
                </span>
                {template.isNew ? (
                  <span className="rounded-md border border-emerald-400/20 bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold text-emerald-300">
                    New listing
                  </span>
                ) : (
                  <span className="rounded-md border border-white/10 bg-white/[0.05] px-2.5 py-1 text-xs font-semibold text-slate-300">
                    Popular direction
                  </span>
                )}
              </div>
              <h1 className="mt-4 text-balance text-[40px] font-bold leading-[1.05] text-white sm:text-[54px]">
                {template.name}
              </h1>
              ...
              <div className="edge-glow mt-8 rounded-lg border border-white/12 bg-[#070b12] p-2 sm:p-3">
                <TemplatePreview
                  name={template.name}
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {template.tags.map((tag) => (
                  <span key={tag} className="rounded-md border border-white/12 bg-white/[0.035] px-3 py-2 text-sm font-medium text-slate-400">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <div className="app-panel rounded-lg p-5 sm:p-6">
                <div className="flex items-end justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-4xl font-bold text-white">${template.price}</p>
                  </div>
                ...
                <div className="mt-6 grid grid-cols-3 divide-x divide-white/10 border-y border-white/10 py-4">
                  <div className="pr-3">
                    <p className="text-lg font-bold text-slate-100">{template.creator.rating.toFixed(1)}</p>
                    <p className="mt-1 text-xs text-slate-500">creator rating</p>
                  </div>
                  <div className="px-3">
                    <p className="text-lg font-bold text-slate-100">{template.creator.completedProjects}</p>
                    <p className="mt-1 text-xs text-slate-500">projects</p>
                  </div>
                  <div className="pl-3">
                    <p className="text-sm font-bold text-slate-100">{template.creator.deliveryTime}</p>
      ...
      <section className="app-section-raised px-5 py-14 sm:px-6 sm:py-18 lg:px-8">
        <div className="mx-auto max-w-[1280px]">
      ...
      <section className="app-section border-t border-white/10 px-5 py-14 sm:px-6 sm:py-18 lg:px-8">
        <div className="mx-auto max-w-[1280px]">
  ...
  return (
    <article className="app-panel-soft rounded-lg p-5">
      ...
      <ul className="mt-4 divide-y divide-white/10">
        {items.map((item) => (
          <li key={item} className="flex gap-3 py-3 text-sm leading-6 text-slate-400 first:pt-0 last:pb-0">
